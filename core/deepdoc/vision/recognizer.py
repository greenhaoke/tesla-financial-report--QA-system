...
from core.api.utils.file_utils import get_project_base_directory
    ...
    def __init__(self, label_list, task_name, model_dir=None):
        ...
        if 'CUDAExecutionProvider' in available_providers:
            try:
                ...
                print(f"[Recognizer] Model '{task_name}' loaded on GPU (CUDA)")
            except Exception as e:
                print(f"[Recognizer] GPU load failed ({e}), falling back to CPU")
                self.ort_sess = ort.InferenceSession(model_file_path, providers=['CPUExecutionProvider'])
        else:
            self.ort_sess = ort.InferenceSession(model_file_path, providers=['CPUExecutionProvider'])
            print(f"[Recognizer] Model '{task_name}' loaded on CPU (CUDA not available)")
